...
const resolveScriptPath = () => {
   ...
   // ML script nằm tại be/ml/ — từ recommendation-service cần đi lên 3 cấp
   return path.resolve(__dirname, "..", "..", "..", "..", "ml", "next_purchase_recommender.py");
};
...
export const getNextPurchasePredictions = async (
   ...
   try {
      ...
      // predict-only failed → fall through to full training
   }
...
// ---------------------------------------------------------------------------
// Cache invalidation — được gọi bởi order-service qua HTTP
// ---------------------------------------------------------------------------
